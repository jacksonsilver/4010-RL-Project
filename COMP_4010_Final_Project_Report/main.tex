...
Thin Ice is more than just a puzzle game. Its simple rules hide interesting challenges that make it a good environment for studying reinforcement learning. Because each tile melts after being stepped on, the agent has to plan their moves ahead instead of moving randomly. This allows the problem at hand to become similar to finding a Hamiltonian path, a route that visits each time once. Working on Thin Ice will give us a chance to see how RL can deal with limited choices, deal with risks and discover paths that paths that cover a whole space, making this project useful both for learning about RL in theory and for applying it to practical applications.
