...
class Level:
    def __init__(self, level_str: str):
        ...
        level_file_path = os.path.join(PATH_TO_LEVELS, level_str)
            
        self._tiles = self.generate_tiles(level_file_path)
    ...
    def perform_action(self, action: PlayerActions):         
        ...
        if new_tile is not None and new_tile.tile_type not in (LevelTileType.WALL, LevelTileType.WATER, LevelTileType.BLANK):
    ...
    def get_available_actions(self, tile: Tile) -> int:
        ...
        for action in PlayerActions:
            ...
            new_tile = self.get_tile(new_pos)
            # Action is available if it's on the grid and not a wall, water, or blank
            if new_tile is not None and new_tile.tile_type not in (LevelTileType.WALL, LevelTileType.WATER, LevelTileType.BLANK):
                mask |= (1 << action.value)
        return mask
